...
deptos_centroids = st_centroid(deptos_RAO) #para etiquetar poligonos en ggplot
...
ggplot() +
  geom_sf(data = deptos_RAO, aes(fill = RAO), color = NA )+
...
# En el dashboard, 
ggplot()+
  geom_sf(data = deptos_RAO %>% filter(DEPARTAMENTO == "Meta"), 
          aes(fill = RAO),
          color = NA)+
  geom_label_repel( data = deptos_centroids %>% 
                   ...
                   segment.ncp = 10,
                   show.legend = FALSE) +
  ggsn::scalebar(data = deptos_RAO %>% filter(DEPARTAMENTO == "Meta"), 
           dist = 50, 
           dist_unit = "km",
           transform = TRUE,
           st.dist = 0.05,
