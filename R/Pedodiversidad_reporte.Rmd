...
caqueta_RAO <- left_join(caqueta_nucleo, RAO_nucleos %>% filter(DEPARTAMENTO == "Caqueta"), by = c("UCS_F", "DEPARTAMENTO"))
...
ggplot()+
  geom_sf(data = nucleos %>% filter(str_detect(OBSERV, "Nucleo 2")), fill = "grey50", color = NA, alpha = 0.5) +
  geom_sf(data = meta_RAO, aes(fill = RAO), color = NA) +
  scale_fill_viridis()+
  theme_minimal()
