...
RAO_out <- read_csv("https://raw.githubusercontent.com/cmguiob/TERRAE_PECIG_dashaboard/main/Resultados/Pedodiversidad_RAO_nucleos_1_2.csv") %>%
...
meta_RAO <- inner_join(meta_nucleo, RAO_out %>% filter(DEPARTAMENTO == "Meta"), by = c("UCS_F", "DEPARTAMENTO", "NUCLEO")) %>%
                  distinct(., geometry, .keep_all = TRUE)

guaviare_RAO <- inner_join(guaviare_nucleo, RAO_out %>% filter(DEPARTAMENTO == "Guaviare"), by = c("UCS_F", "DEPARTAMENTO", "NUCLEO")) %>%
                  distinct(., geometry, .keep_all = TRUE)

vichada_RAO <- inner_join(vichada_nucleo, RAO_out %>% filter(DEPARTAMENTO == "Vichada"), by = c("UCS_F", "DEPARTAMENTO", "NUCLEO")) %>%
                  distinct(., geometry, .keep_all = TRUE)

caqueta_RAO <- inner_join(caqueta_nucleo, RAO_out %>% filter(DEPARTAMENTO == "Caqueta"), by = c("UCS_F", "DEPARTAMENTO", "NUCLEO")) %>%
                  distinct(., geometry, .keep_all = TRUE)
...
deptos_centroids <- rbind(meta_centroids, 
                          guaviare_centroids, 
                          vichada_centroids, 
                          caqueta_centroids) 
...
plot_EDA <- ggplot(data = deptos_RAO %>% 
                     st_drop_geometry(), 
...
ggplot()+
  geom_sf(data = deptos_RAO %>% filter(DEPARTAMENTO == "Guaviare"), 
          aes(fill = RAO),
          color = NA)+
  geom_label_repel( data = guaviare_centroids %>% 
...
ggplot(data = deptos_RAO %>% 
    ...
    geom_hline(aes(yintercept = 0.5), 
               ...
               size = 0.5, 
               linetype = "dashed")+
    stat_summary(fun = mean, geom = "point", size = 5, shape = 18, color = "grey20")+
    coord_flip() +
    guides(color = "none",
